...
Running the axis_fifo VHDL simulation (ModelSim, GUI) -- two ways:
  ...
  1. Via this runner, from the fpga-ip repo root:
       run axis_fifo vhdl modelsim gui
        ...
        Or from an IP directory containing scripts/*.f:
            run . vhdl modelsim gui
...
class InvocationContext:
    """Resolved filesystem context for one runner invocation."""
...
def _display_path(path: Path) -> str:
    """Return a repository-relative path, or an absolute external path."""
    try:
...
def _tcl_source_path(path: Path) -> str:
    """Return a Tcl path usable from a generated tool script."""
    try:
...
def _auto_run_dir(ip: str, tool: str, ip_root: Path | None = None) -> Path:
    ...
    return (ip_root or (REPO_ROOT / ip)) / RUNS_DIRNAME / tool_dir


def _persistent_project_dir(value: str, ip: str,
                            ip_root: Path | None = None) -> Path:
    ...
    """
    root = (ip_root or (REPO_ROOT / ip)).resolve()
    path = Path(value)
    if path.is_absolute():
        path = path.resolve()
    else:
    ...
    try:
        ...
        raise RunnerError("--project-dir must be below the active IP directory") from exc
    if RUNS_DIRNAME in path.relative_to(root).parts:
...
def parse_manifest(path: Path, source_root: Path | None = None) -> Manifest:
    ...
    for raw in path.read_text(encoding="utf-8-sig").splitlines():
        ...
        if tokens[0].lower() == "include":
            ...
            current.entries.append(IncludeEntry(
                path=(source_root / inc_path).resolve(),
        ...
        entry_path = (source_root / tokens[0]).resolve()
        std = attrs.get("std")
    ...
    return Manifest(path=path, source_root=source_root, directives=directives,
    ...
    try:
        for entry in manifest.sections[section_name].entries:
            if isinstance(entry, IncludeEntry):
                ...
                if inc is None:
                    inc = parse_manifest(entry.path, manifest.source_root)
...
def _compile_lines(files: list[FileEntry], indent: str = "") -> list[str]:
    ...
    out: list[str] = []
    for entry in files:
        source_path = _tcl_source_path(entry.path)
        label = _display_path(entry.path)
        if entry.path.suffix.lower() in (".vhd", ".vhdl"):
            flag = STD_TO_VCOM.get(entry.std or "2008", "-2008")
            out.append(f'{indent}vcom {flag} -work work "{source_path}"')
        else:
            out.append(f'{indent}vlog -work work "{source_path}"')
    return out
...
def write_msim_script(script_path: Path, files: list[FileEntry], top: str,
    ...
    if mode != "batch":
        run_cmd += f" {mode}"
    run_dir = _display_path(script_path.parent)
    lines = ["# ============================================================================",
    ...
    if is_project:
        ...
        for entry in files:
            source_path = _tcl_source_path(entry.path)
            lines.append(f'project addfile [file nativename [file normalize "{source_path}"]]')
...
def write_xsim_script(script_path: Path, files: list[FileEntry],
    ...
    for entry in files:
        ...
        lines.append(f"  set {source_var} [file normalize \"{source_path}\"]")
        if entry.path.suffix.lower() in (".vhd", ".vhdl"):
        ...
        lines.append(f"  run_external \"compile {label}\" {command}")
...
def _vivado_read_lines(files: list[FileEntry], indent: str = "") -> list[str]:
    ...
    out: list[str] = []
    for e in files:
        source_path = _tcl_source_path(e.path)
        if e.path.suffix.lower() == ".xdc":
            out.append(f'{indent}read_xdc "{source_path}"')
        elif e.path.suffix.lower() in (".vhd", ".vhdl"):
            info = _VIVADO_STD.get(e.std or "2008", _VIVADO_STD["2008"])
            out.append(f'{indent}read_vhdl{info["read_flag"]} "{source_path}"')
        else:
            out.append(f'{indent}read_verilog -sv "{source_path}"')
    return out
...
def write_vivado_project_script(script_path: Path, files: list[FileEntry],
    ...
    for e in files:
        ...
        source_path = _tcl_source_path(e.path)
        body.append(f'set f [add_files -norecurse -fileset sources_1 "{source_path}"]')
    ...
    if xdcs:
        ...
        for e in xdcs:
            source_path = _tcl_source_path(e.path)
            body.append(f'add_files -norecurse -fileset constrs_1 "{source_path}"')
    if sim_files:
        body += ["",
        ...
        for e in sim_files:
            source_path = _tcl_source_path(e.path)
            body.append(f'set f [add_files -norecurse -fileset sim_1 "{source_path}"]')
            body.append(f'set_property file_type "{_vivado_file_type(e)}" $f')
    body += ["",
...
def cmd_clean(argv: list[str]) -> int:
    ...
    p.add_argument("ip", help="IP directory under the repo root, or '.' for the current IP")
    args = p.parse_args(argv)

    removed = []
    if args.ip == ".":
        try:
            runs_dir = _resolve_current_ip_root() / RUNS_DIRNAME
...
def _resolve_invocation_context(args, manifest_name: str) -> InvocationContext:
    """Resolve the manifest and source root for a repository or local IP."""
    if args.ip == ".":
        ip_root = _resolve_current_ip_root()
        return InvocationContext(
            ip=".",
            ip_root=ip_root,
            manifest_path=ip_root / "scripts" / manifest_name,
            source_root=ip_root,
        )

    ip_root = (REPO_ROOT / args.ip).resolve()
    try:
        ip_root.relative_to(REPO_ROOT)
    except ValueError as exc:
        raise RunnerError(f"IP path escapes repo root: {args.ip}") from exc
    return InvocationContext(
        ip=args.ip,
        ip_root=ip_root,
        manifest_path=ip_root / "scripts" / manifest_name,
...
def _resolve_sweep_dot() -> str:
    """Resolve '.' to the current direct-child IP name for a sweep."""
    relative = _resolve_current_ip_root().relative_to(REPO_ROOT)
    return relative.as_posix()


def _resolve_current_ip_root() -> Path:
    """Return the current direct-child IP directory or raise a clear error."""
    current = Path.cwd().resolve()
    try:
        relative = current.relative_to(REPO_ROOT)
    except ValueError as exc:
        raise RunnerError("'.' must refer to an IP directory containing scripts/*.f") from exc
    if len(relative.parts) != 1 or not (current / "scripts").is_dir():
        raise RunnerError("'.' must refer to an IP directory containing scripts/*.f")
    return current

def build_arg_parser() -> argparse.ArgumentParser:
    """Construct the argparse CLI parser.

    Positionals are <ip> <manifest> <tool> [mode]. Use '.' as <ip> when
    invoking from an IP directory.
    Mode defaults to 'batch'.
    ...
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  cd axis_fifo && run . vhdl modelsim  # from an IP directory\n"
            "  cd axis_fifo && run . all all        # sweep the current IP\n"
            "  run clean axis_fifo"
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        context = _resolve_invocation_context(args, manifest_name)
    except RunnerError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2

    try:
        manifest = parse_manifest(context.manifest_path, context.source_root)
    except RunnerError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2
    ...
    for tb_name in tb_names or [None]:
        ...
        if tb_name is not None and tool in MSIM_TOOLS:
            ...
            if mode == "project" and args.project_dir:
                try:
                    build_dir = _persistent_project_dir(
                        args.project_dir, args.ip, context.ip_root)
                except RunnerError as exc:
                    print(f"ERROR: {exc}", file=sys.stderr)
                    return 2
            else:
                build_dir = _auto_run_dir(args.ip, tool, context.ip_root)
            ...
            if not _QUIET:
                print(f"script   : {_display_path(script_path)}")
