...
Remove build artifacts for an IP (deletes <ip>/sim/, <ip>/sim_proj/, and
<ip>/xsim/):
       run clean <ip>
...
def report_sim_result(mode: str, rc: int) -> int:
    """Print the result line for a simulated target; return exit-code delta.
...
def cmd_clean(argv: list[str]) -> int:
    ...
    project), and <ip>/xsim/ (XSim build: xsim.dir and logs). A directory
    held open by a running tool is reported and left in place.
    """
    p = argparse.ArgumentParser(
        prog="run clean",
        description="Remove an IP's build artifacts (the <ip>/sim/, <ip>/sim_proj/, and <ip>/xsim/ directories)",
    )
    p.add_argument("ip", help="IP directory under the repo root")
    args = p.parse_args(argv)

    removed = []
    for name in ("sim", "sim_proj", "xsim"):
    ...
    if not removed:
        print(f"nothing to clean: no sim/, sim_proj/, or xsim/ under {args.ip}/")
        return 0
